...
import com.github.smeny.jpc.emulator.*;
  ...
  public void updateNowAndProcess(boolean sleep) {
    if (REAL_TIME) {
      ...
      if (nanoDelay > 0) {
        ...
        totalTicks +=
            convertNanosToTicks(nanoDelay) + 1; // only place where ticks gets out of sync with number of instructions
      }
      ...
      totalTicks = (long) ((double) expiry * IPS /
          getTickRate());//totalTicks += ((expiry - getEmulatedNanos())/1000)*1000 * IPS / getTickRate();
      if (totalTicks < 0) {
