...
import com.github.smeny.jpc.emulator.Hibernatable;
 ...
 *
 * @author Chris Dennis
 * @see <a href="http://pdos.csail.mit.edu/6.828/2007/readings/hardware/8237A.pdf"> 8237A - Datasheet</a>
 */
public class DMAController extends AbstractHardwareComponent implements IODevice {
  ...
  private static final int CMD_NOT_SUPPORTED =
      COMMAND_MEMORY_TO_MEMORY | COMMAND_ADDRESS_HOLD | COMMAND_COMPRESSED_TIMING | COMMAND_CYCLIC_PRIORITY |
          COMMAND_EXTENDED_WRITE | COMMAND_DREQ_SENSE_LOW | COMMAND_DACK_SENSE_LOW;
  ...
  public class DMAChannel implements Hibernatable {
    ...
    /**
     * Reads memory from this channel. <p> Allows a <code>DMATransferCapable</code> device to read the section of memory
     * currently pointed to by this channels internal registers.
     *
    ...
    /**
     * Writes data to this channel. <p> Allows a <code>DMATransferCapable</code> device to write to the section of
  ...
  /**
   * Constructs a DMA controller (either primary or secondary).  If <code>highPageEnable</code> is true then 32 bit
   * addressing is possible, otherwise the controller is limited to 24 bits.
   *
  ...
  public void loadState(DataInput input) throws IOException {
    ...
    pageLowBase = input.readInt();
    pageHighBase = input.readInt();
    controllerNumber = input.readInt();
    int len = input.readInt();
    dmaChannels = new DMAChannel[len];
    for (int i = 0; i < dmaChannels.length; i++) {
      dmaChannels[i] = new DMAChannel();
  ...
  /**
   * Returns true if this controller is the primary DMA controller. <p> Non-primary or secondary controllers operate by
   * being chained off the primary controller.
   *
  ...
  /**
   * Request a DMA transfer operation to occur on the specified channel. <p> This is equivalent to pulling the DREQ line
   * high on the controller.
   *
   * @param channel channel index.
   */
  ...
  /**
   * Request the DMA transfer in operation on the specified channel to stop. <p> This is equivalent to pulling the DREQ
   * line low on the controller.
   *
   * @param channel channel index.
   */
  ...
  /**
   * Register the given <code>DMATransferCapable</code> device with the specified channel. <p> Subsequent DMA requests
   * on this channel will call the <code>handleTransfer</code> method on <code>device</code>.
   *
   * @param channel channel index.
