/*
 * JPC: An x86 PC Hardware Emulator for a pure Java Virtual Machine Release Version 2.4
 * 
 ...
 * 
 * jpc.sourceforge.net or the developer website sourceforge.net/projects/jpc/
 * 
 * Conceived and Developed by: Rhys Newman, Ian Preston, Chris Dennis
 * 
...
public final class PhysicalAddressSpace extends AddressSpace implements HardwareComponent {
  ...
  /**
   * Constructs an address space which is initially empty. All addresses are mapped to an instance
   * of the inner class <code>UnconnectedMemoryBlock</code> whose data lines float high.
   */
  public PhysicalAddressSpace(int systemRamSize, CodeBlockManager manager) {
    this.systemRamSize = systemRamSize;
    this.quickIndexSize = systemRamSize >>> INDEX_SHIFT;
  ...
  private static void dumpMemory(DataOutput output, Memory[] quick, Memory[][] full)
      throws IOException {
  ...
  private static void loadMemory(DataInput input, Memory[] quick, Memory[][] full,
      CodeBlockManager manager) throws IOException {
    byte[] temp = new byte[0];
  ...
  private void initialiseMemory() {
    for (int i = 0; i < systemRamSize; i += AddressSpace.BLOCK_SIZE) {
      mapMemory(i, new LazyCodeBlockMemory(AddressSpace.BLOCK_SIZE, manager));
    }
    // memory hole, the last 64 K of this is replaced by BIOS shadow ram if the BIOS ROM is 128 K or
    // greater
  ...
  public int executeReal(Processor cpu, int offset) {
    try {
      ...
      getReadMemoryBlockAt(offset + 0x1000).addSpanningBlock(block,
          length - (0x1000 - (offset & AddressSpace.BLOCK_MASK)));
      return getReadMemoryBlockAt(offset).executeReal(cpu, offset & AddressSpace.BLOCK_MASK);
    }
  }
  ...
  public void unmap(int start, int length) {
    if ((start % BLOCK_SIZE) != 0) {
      throw new IllegalStateException(
          "Cannot deallocate memory starting at " + Integer.toHexString(start)
  ...
  public void mapMemoryRegion(Memory underlying, int start, int length) {
    if (underlying.getSize() < length) {
      throw new IllegalStateException("Underlying memory (length=" + underlying.getSize()
          + ") is too short for mapping into region " + length + " bytes long");
    }
    if ((start % BLOCK_SIZE) != 0) {
    ...
    if ((length % BLOCK_SIZE) != 0) {
      throw new IllegalStateException(
          "Cannot map memory in partial blocks: " + length + " is not a multiple of " + BLOCK_SIZE);
    }
    unmap(start, length);
    if (Option.log_memory_maps.isSet())
      if (((start & 0xffffffffL) > systemRamSize) || !(underlying instanceof LazyCodeBlockMemory))
  ...
  public void mapMemory(int start, Memory block) {
    if ((start % BLOCK_SIZE) != 0) {
      throw new IllegalStateException("Cannot allocate memory starting at "
          + Integer.toHexString(start) + "; this is not aligned to " + BLOCK_SIZE + " bytes");
    }
  ...
  public static final class UnconnectedMemoryBlock implements Memory {
    ...
    public int executeReal(Processor cpu, int offset) {
      throw new IllegalStateException(
          "Trying to execute in Unconnected Block @ 0x" + Integer.toHexString(offset));
    }
  ...
  private void setMemoryBlockAt(int i, Memory b) {
    try {
      ...
      try {
        nonA20MaskedIndex[i >>> TOP_INDEX_SHIFT][(i >>> BOTTOM_INDEX_SHIFT) & BOTTOM_INDEX_MASK] =
            b;
      } catch (NullPointerException n) {
