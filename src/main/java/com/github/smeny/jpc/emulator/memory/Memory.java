...
/**
 * A region of memory that can be read from and written to in all sizes from byte to quad-word.  Also supports execution
 * from an arbitrary point on a given processor state.
 *
 * @author Chris Dennis
 */
public interface Memory {
  ...
  /**
   * Returns true if this <code>Memory</code> object has had heap allocated for it. <p> For most memory objects a
   * <code>true</code> return implies that there are some non-zero values stored.
   *
  ...
  /**
   * Sets <code>length</code> bytes to zero from <code>start</code> (inclusive) to <code>(start + length)</code>
   * (exclusive).
   *
  ...
  /**
   * Sets the least significant 64bits of an octa-word value starting at <code>index</code> in little-endian format.
   *
   * @param offset index of the first byte.
  ...
  /**
   * Sets the most significant 64bits of an octa-word value starting at <code>index</code> in little-endian format.
   *
   * @param offset index of the first byte.
  ...
  /**
   * Copies <code>len</code> bytes starting at <code>address</code> from this memory object into <code>buffer</code>.
   *
  ...
  /**
   * Copies <code>len</code> bytes starting at <code>off</code> from <code>buffer</code> into this memory object at
   * <code>address</code>.
   *
  ...
  /**
   * Copies <code>len</code> bytes starting at <code>off</code> from <code>buffer</code> into this memory object at
   * <code>address</code>, but does not initialise code block arrays.
   *
   * @param address start address to copy to.
