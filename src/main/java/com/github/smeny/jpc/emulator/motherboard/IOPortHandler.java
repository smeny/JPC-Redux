...
/**
 * This class holds the map between ioport addresses and <code>IODevice</code> objects.  Unmapped ports are redirected
 * to an unconnected inner class instance whose data lines float high, and on which writes fail silently.
 *
 * @author Chris Dennis
 */
public class IOPortHandler extends AbstractHardwareComponent implements IODevice {
  ...
  /**
   * Constructs a new <code>IOPortHandler</code> with an initially empty ioport mapping.  All ioports map to the
  ...
  /**
   * Map an <code>IODevice</code> device into this handler. <p> The range of ioports requested by this device are
   * registered with the handler.  Each individual port is registered only if that port is currently unconnected.
   *
  ...
  public void registerIOPortCapable(IODevice device) {
    ...
    for (int port : portArray) {
      if (ioPortDevice[port] == defaultDevice) ioPortDevice[port] = device;
    }
  }

  /**
   * Unmap an <code>IODevice</code> device from this handler. <p> Ports are only unmapped from the handler if they are
   * currently to the supplied object.  References to other objects at the addresses claimed are not cleared.
   *
  ...
  public void deregisterIOPortCapable(IODevice device) {
    int[] portArray = device.ioPortsRequested();
    for (int port : portArray) {
      if (ioPortDevice[port] == device) ioPortDevice[port] = defaultDevice;
    }
  }
