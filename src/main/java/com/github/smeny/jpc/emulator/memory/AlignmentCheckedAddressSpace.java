...
/**
 * Class that implements an alignment checking skin on another <code>AddressSpace</code> instance.  Access that are not
 * alignment on the correct granularity will trigger a {@link  com.github.smeny.jpc.emulator.processor.ProcessorException}
 * with the appropriate vector value.
 *
 * @author Chris Dennis
 */
public class AlignmentCheckedAddressSpace extends AddressSpace {
  ...
  /**
   * Throws a <code>ProcessorException</code> if the access is not aligned to a word (two-byte) boundary.
   *
   * @param offset address to be read.
   * @return short value read.
   */
  public short getWord(int offset) {
    if ((offset & 0x1) != 0) throw ProcessorException.ALIGNMENT_CHECK_0;

    return addressSpace.getWord(offset);
  }

  /**
   * Throws a <code>ProcessorException</code> if the access is not aligned to a doubleword (four-byte) boundary.
   *
   * @param offset address to be read.
   * @return int value read.
   */
  public int getDoubleWord(int offset) {
    if ((offset & 0x3) != 0) throw ProcessorException.ALIGNMENT_CHECK_0;

    return addressSpace.getDoubleWord(offset);
  }

  /**
   * Throws a <code>ProcessorException</code> if the access is not aligned to a quad-word (eight-byte) boundary.
   *
   * @param offset address to be read.
   * @return int value read.
   */
  public long getQuadWord(int offset) {
    if ((offset & 0x7) != 0) throw ProcessorException.ALIGNMENT_CHECK_0;

    return addressSpace.getQuadWord(offset);
  }

  /**
   * Throws a <code>ProcessorException</code> if the access is not aligned to a octa-word (sixteen-byte) boundary.
   *
   * @param offset address to be read.
   * @return long value read.
   */
  public long getLowerDoubleQuadWord(int offset) {
    if ((offset & 0xF) != 0) throw ProcessorException.ALIGNMENT_CHECK_0;
  ...
  /**
   * Throws a <code>ProcessorException</code> if the access is not aligned to a quadword (eight-byte) boundary.
   *
   * @param offset address to be read.
   * @param data   value to write.
   */
  ...
  public void setLowerDoubleQuadWord(int offset, long data) {
    if ((offset & 0xF) != 0) throw ProcessorException.GENERAL_PROTECTION_0;
